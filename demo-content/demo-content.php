...
class RTDemoimport {
	...
	public function __construct() {
		...
		add_action( 'ocdi/before_content_import', [$this,'ocdi_before_content_import'] );
		...
		if ( isset( $_GET['shopbuilder_settings'] ) && $_GET['shopbuilder_settings'] == 1 ) {
			$this->shopbuilder_settings_export();
		}
	}

	public function ocdi_before_content_import(){
		$pagesToDelete = [
			'My Account',
			'Checkout',
			'Cart',
		...
		foreach ( $pagesToDelete as $pageTitle ) {
			$page = $this->get_page_by_title( $pageTitle );
	...
	public function custom_admin_css( $hook_suffix ) {
		if ( 'appearance_page_install_demos' === $hook_suffix ) {
			wp_enqueue_style( 'custom-admin-css', FASHENO_CORE_BASE_URL . '/demo-content/css/main.css', [], '1.0.0' );
		}
	}
	...
	public function shopbuilder_settings_export(){
		$option_names = array('rtsb_settings', 'rtsb_extra_settings', 'rtsb_template_settings');
		$data = array();
		...
		file_put_contents( RDTHEME_CORE_DEMO_CONTENT . 'demo/shopbuilder-settings.json', $json_data );
	}

	public function shopbuilder_settings_import() {

		$json_file_path = RDTHEME_CORE_DEMO_CONTENT . 'demo/shopbuilder-settings.json';
		$fileExists  = file_exists( $json_file_path );
		if ($fileExists){
	...
	private function change_attribute_type() {
		global $wpdb;
	...
	public function import_page_setup( $default_settings ) {
		...
		$default_settings['page_title']  = esc_html__( 'Import Demo Data', 'fasheno-core' );
		$default_settings['menu_title']  = esc_html__( 'Import Demo Data', 'fasheno-core' );
	...
	public function intro_text( $default_text ) {
        ...
        <div class="fasheno-core_intro-text vtdemo-one-click">
            <div id="poststuff">
                ...
                <div class="postbox vt-support-box">
                    ...
                    <div class="inside">
                        ...
                        <a href="https://themeforest.net/item/fasheno/123456?ref=rt" class="button-primary" target="_blank">Item Page</a>
                    </div>
                </div>
...
add_action( 'plugins_loaded', 'fasheno_demo_importer_init' );
/**
 * Initializes the Quixa demo importer.
 *
 * @return RTDemoimport
 */
function fasheno_demo_importer_init() {
