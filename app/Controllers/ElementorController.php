...
use RT\FashenoCore\Elementor\Core\ElementorCore;
use RT\FashenoCore\Modules\IconList;
use RT\FashenoCore\Elementor\Widgets\Title;
use RT\FashenoCore\Elementor\Widgets\Button;
...
class ElementorController {
	...
	public function editor_scripts() {
		$version = ( defined( 'WP_DEBUG' ) && WP_DEBUG ) ? time() : FASHENO_CORE;

		wp_enqueue_script(
			'fasheno-editor-script',
	...
	public function widget_category( $elements_manager ) {
		$id                = FASHENO_CORE_PREFIX . '-widgets';
		$categories[ $id ] = [
			'title' => __( 'RadiusTheme Elements', 'fasheno-core' ),
			'icon'  => 'fa fa-plug',
		];
	...
	public function fontello_support( $tabs = [] ) {
		...
		$tabs['rt-fontello-icons'] = [
			...
			'label'         => esc_html__( 'Fontello Icons', 'fasheno-core' ),
